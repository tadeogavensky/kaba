...
const EditForm: React.FC<EditFormProps> = ({ closeForm }) => {
  ...
  const [zipPostalCode, setZipPostalCode] = useState(user?.worker?.postalCode);
  const [neighbourhood, setNeighbourhood] = useState(
    user?.worker?.neighbourhood
  );
  const [street, setStreet] = useState(user?.worker?.street);
  const [streetNumber, setStreetNumber] = useState(user?.worker?.number);
  ...
  const handleSubmitAddressForm = async (e: FormEvent) => {
    ...
    try {
      const response = await axios.put("/api/address", address);
